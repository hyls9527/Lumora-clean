...
- **发布签名前提**：tauri v2 必须 `tauri.conf.json` 的 `bundle.updater.active = true` 才会生成 `.sig` 文件。发布后必须验证：release 资产含 `.sig`、`latest.json` 的 signature 非空（v0.8.1~v0.10.1 曾长期缺失该配置，自动更新签名验证静默失效；updater-json job 现已加签名空检查，签名缺失会阻断发布）。
