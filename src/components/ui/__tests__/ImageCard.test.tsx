...
describe('ImageCard — 灯箱印样', () => {
  ...
  it('shows plum rating stamp when rating > 0', () => {
    const { container } = render(<ImageCard image={MOCK_IMAGE} />);
    const stamp = container.querySelector('.image-card__stamp--rate');
    expect(stamp).not.toBeNull();
    expect(stamp?.querySelector('svg')).not.toBeNull();
    expect(stamp?.textContent).toContain('3');
  });

  it('hides rating stamp when rating is 0', () => {
    const { container } = render(<ImageCard image={{ ...MOCK_IMAGE, rating: 0 }} />);
    expect(container.querySelector('.image-card__stamp--rate')).toBeNull();
  });
