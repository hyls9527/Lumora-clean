...
**Development not started — design prototypes complete (2026-06-25)**
...
- Cloud sync — local-first desktop app (deferred)
- Duplicate detection — deferred
- Backup/restore — deferred

## Current Milestone: v0.1 — MVP Frontend

**Status:** Development not started. Design prototypes (HTML) and documentation complete.

**Goal:** Build the core image management frontend — gallery, curation, command palette, keyboard navigation, and rating/favoriting.
...
**IPC:** stdin/stdout JSON-RPC | **Vector store:** sqlite-vec + exhaustive KNN | **Ollama:** optional, CLIP is core

**Target features:**
- SQLite persistence with versioned schema migrations
- FTS5 full-text search replacing mock filtering
- CLIP embedding generation via Python sidecar
- sqlite-vec vector search powering semantic search
- Ollama vision model integration for AI analysis
- Desktop installer (Windows .msi)
...
| Mock data for all images | No backend available; enables full UI development | ✓ Resolved in v0.3 (real SQLite persistence) |
| 200ms transitions, 200px sidebar, 3-layer shadows | From DESIGN.md — consistent,克制 aesthetic | ✓ Good |

---
*Last updated: 2026-06-23 — v0.3 milestone complete*
