...
/// When `field` is None or "all", delegates to FTS5 MATCH.
/// When `field` is a specific key (prompt, seed, model, etc.),
/// uses json_extract + LIKE for precise matching.
#[tauri::command]
pub fn search_images_advanced(
    ...
    // Seed is numeric — use exact match
    if field == "seed" {
    ...
    // Text fields — use LIKE on json_extract
...
mod tests {
    ...
    fn scan_folder_handles_single_file() {
        // Create a temp file to test single file import
        let temp_dir = std::env::temp_dir();
        let test_file = temp_dir.join("lumora_test_single.png");
        
        // Create a minimal PNG file (8x8 red pixel)
    ...
    fn scan_folder_rejects_non_image_file() {
        ...
        assert!(result.is_ok());
        let entries = result.unwrap();
        assert_eq!(entries.len(), 0); // Should be empty for non-image files
